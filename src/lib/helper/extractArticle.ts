...
function extractArticle(element: Element) {
	...
	const year =
		parseInt(element.querySelector('.gsc_a_y .gsc_a_hc')?.textContent?.trim() || '') || 0;
...
export function parseArticle(html: string, sortOrder: 'asc' | 'desc' = 'desc') {
	...
	const articles = Array.from(document.querySelectorAll('#gsc_a_t > #gsc_a_b > .gsc_a_tr')).map(
		(items) => extractArticle(items)
	);

	return articles.sort((a, b) => (sortOrder === 'asc' ? a.year - b.year : b.year - a.year));
}
